...
#include "../oplus/oplus_display_esd.h"
...
#ifdef OPLUS_FEATURE_DISPLAY
	if (display_type == DSI_PRIMARY)
		oplus_panel_get_id(display, boot_str);
#endif /* OPLUS_FEATURE_DISPLAY */
...
#ifdef OPLUS_FEATURE_DISPLAY
		if(!strcmp(boot_displays[i].name, "qcom,mdss_dsi_panel_AC052_P_3_A0003_dsc_cmd_dvt")) {
			boot_displays[i].name[strlen(boot_displays[i].name) - 4] = '\0';
		}
#endif /* OPLUS_FEATURE_DISPLAY */
...
static struct device_node *_iris_dsi_display_get_panel_node(struct platform_device *pdev, int index, bool boot_disp_en, bool is_sencondary)
{
	...
	if (disp_name) {
		...
		if (boot_disp_en) {
			strlcpy(boot_displays[index].name, disp_name,
					MAX_CMDLINE_PARAM_LEN);
			boot_displays[index].boot_disp_en = true;
			disp_name = NULL;
		} else {
			return of_find_node_by_name(mdp_node, disp_name);
		}
	}
			...
			struct dsi_display_mode *adj_mode)
{
	...
	if (display->panel->oplus_priv.ffc_enabled &&
			...
			display->panel->oplus_priv.ffc_delay_frames) {
