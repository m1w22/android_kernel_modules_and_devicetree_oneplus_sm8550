...
static struct tzdbg tzdbg = {
	...
	.stat[TZDBG_APDP_CFG].name = "apdp_cfg",
};
...
static int _disp_apdp_cfg_stats(void)
{
	char apdp_cfg_buf[APDP_CFG_BUF_LEN] = {0};
	struct qseecom_scm_desc desc = {0};
	int ret = 0;
	desc.args[0] = 0;
	desc.arginfo = 0;
	ret = qcom_scm_qseecom_call(SCM_SIP_FNID(SCM_SVC_UTIL, TZ_DUMP_SECURITY_ALLOWS_MEM_DUMP), &desc, false);
	if (ret) {
		pr_err("%s: SCM call failed\n", __func__);
		return 0;
	}
	ret = sprintf(apdp_cfg_buf, "%s\n%s\n%s\n",
	(desc.ret[0] == 0) ? "Dumps not Allowed" : ((desc.ret[0] == 1) ? "SEC_DMP_ALLOWED" : "NSEC_DMP_ALLOWED"),
	(desc.ret[1] == 0) ? "DLOAD Cookie NOT set" : "DLOAD Cookie set",
	(desc.ret[2] == 0) ? "Apps Non-Secure (HLOS) Minidump disenabled":"Apps Non-Secure(HLOS) Minidump enabled");
	tzdbg.stat[TZDBG_APDP_CFG].data = apdp_cfg_buf;
	pr_info("%s: len=%d, resp0 = %d, resp1 = %d, resp2 = %d\n", __func__, ret, desc.ret[0], desc.ret[1], desc.ret[2]);
	pr_info("%s: apdp_cfg=\n%s", __func__, apdp_cfg_buf);
	return ret;
}
