...
((unsigned int[]) { (unsigned int)WCD939X_##register_name, \
(unsigned int)FIELD_MASK(register_name, field_name), \
(unsigned int)value << FIELD_SHIFT(register_name, field_name)})

#define SND_SOC_UPDATE_BITS(component, values) \
	do { \
		unsigned int *temp = values; \
		snd_soc_component_update_bits(component, temp[0], temp[1], temp[2]); \
	} while (0)
...
static bool wcd939x_readable_register(struct device *dev, unsigned int reg)
{
	...
	if (wcd939x_reg_access[WCD939X_REG(reg)] & RD_REG)
		return true;
	else
		return false;
}
...
static int wcd939x_init_reg(struct snd_soc_component *component)
{
	...
	if (of_find_property(component->card->dev->of_node,
						"qcom,wcd-disable-legacy-surge", NULL)) {
		SND_SOC_UPDATE_BITS(component,
 ...
 *			<OFFSET1_VAL0 LANE1>, <OFFSET1_VAL5 LANE0>, <OFFSET1_VAL1 LANE0>,
 *			<OFFSET1_VAL1 LANE0>,*UC0*
 *			<OFFSET1_VAL0 LANE1>, <OFFSET1_VAL2 LANE0>, <OFFSET1_VAL1 LANE0>,
 *			<OFFSET1_VAL1 LANE0>, *UC1*
 *			<OFFSET1_VAL1 LANE0>, <OFFSET1_VAL1 LANE0>, <OFFSET1_VAL1 LANE0>,
 *			<OFFSET1_VAL1 LANE0>; *UC2*
 *			<OFFSET1_VAL1 LANE0>, <OFFSET1_VAL1 LANE0>, <OFFSET1_VAL1 LANE0>,
 *			<OFFSET1_VAL1 LANE0>; *UC3*
 */
			...
			struct wcd939x_priv *wcd939x, int mode_2vpk)
{
	...
	if (rc) {
		dev_err_ratelimited(component->dev, "%s: Unable to get impedance for 2Vpk mode",
										 __func__);
		return;
	}
...
static const struct snd_soc_dapm_widget wcd939x_dapm_widgets[] = {
	 ...
	 * dapm doesn't set these dapm input and output widgets as endpoints.
	 */
				   ...
				   loff_t pos)
{
	...
	case WCD939X_VERSION_1_1:
		len = scnprintf(buffer, sizeof(buffer), "WCD939X_1_0\n");
		break;
	case WCD939X_VERSION_2_0:
		len = scnprintf(buffer, sizeof(buffer), "WCD939X_2_0\n");
		break;
	default:
					...
					loff_t pos)
{
	...
	switch (priv->variant) {
	case WCD9390:
		len = scnprintf(buffer, sizeof(buffer), "WCD9390\n");
		break;
	case WCD9395:
		len = scnprintf(buffer, sizeof(buffer), "WCD9395\n");
		break;
	default:
		len = scnprintf(buffer, sizeof(buffer), "VER_UNDEFINED\n");
	}
...
static void wcd939x_update_regmap_cache(struct wcd939x_priv *wcd939x)
{
	...
	if (wcd939x->version >= WCD939X_VERSION_1_1) {
		for (i = 0; i < ARRAY_SIZE(reg_def_1_1); ++i)
			regmap_write(wcd939x->regmap, reg_def_1_1[i].reg, reg_def_1_1[i].def);
	}

	if (wcd939x->version == WCD939X_VERSION_2_0) {
		for (i = 0; i < ARRAY_SIZE(reg_def_2_0); ++i)
