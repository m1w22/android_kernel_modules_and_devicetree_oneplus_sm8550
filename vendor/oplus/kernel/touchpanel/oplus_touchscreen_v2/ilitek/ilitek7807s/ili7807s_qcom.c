...
static int ili_spi_wrapper(u8 *txbuf, u32 wlen, u8 *rxbuf, u32 rlen,
			   bool spi_irq, bool i2c_irq)
{
	int ret = 0;
	int mode = 0, index = 0;
	u8 wdata[128] = {0};
...
void ili_report_gesture_mode(u8 *buf, int len)
{
	...
	case GESTURE_LEFT:
		gc->type  = RIGHT2LEFT_SWIP;
		gc->clockwise = 1;
		break;

	case GESTURE_RIGHT:
		gc->type  = LEFT2RIGHT_SWIP;
		gc->clockwise = 1;
		break;

	case GESTURE_UP:
		gc->type  = DOWN2UP_SWIP;
		gc->clockwise = 1;
		break;

	case GESTURE_DOWN:
		gc->type  = UP2DOWN_SWIP;
		gc->clockwise = 1;
		break;

	case GESTURE_O:
		gc->type  = CIRCLE_GESTURE;
		if (ilits->position_high_resolution == OFF) {
...
int ili_mp_test_handler(char *apk, struct seq_file *s,
			...
			struct auto_testdata *testdata)
{
	...
	ret = ili_mp_test_main(apk, s, message, msg_size,
...
out:
	...
	if (chip_info->fw_upgrade_mode == UPGRADE_IRAM) {
		if (ili_fw_upgrade_handler() < 0) {
			ILI_ERR("FW upgrade failed during mp test\n");
		}

	} else {
		if (ili_reset_ctrl(chip_info->reset) < 0) {
...
{
	atomic_set(&ilits->esd_stat, START);
	tp_touch_btnkey_release(ilits->tp_index);
...
int ili_set_tp_data_len(int format, bool send, u8 *data)
{
	...
	} else if ((atomic_read(&ilits->tp_reset) == END) && (tp_mode == P5_X_FW_AP_MODE ||
...
static void ilitek_ftm_process_extra(void *chip_data)
{
	struct ilitek_ts_data *chip_info = (struct ilitek_ts_data *)chip_data;
	int ret = 0;

	if (chip_info != NULL && chip_info->ts != NULL) {
		ILI_INFO("chip_info->ts->boot_mode = %d\n", chip_info->ts->boot_mode);
#ifdef CONFIG_TOUCHPANEL_MTK_PLATFORM

		if ((chip_info->ts->boot_mode == META_BOOT
				|| chip_info->ts->boot_mode == FACTORY_BOOT))
#else
				 ...
				 int is_suspended)
{
	...
	if ((gesture_enable == 1) && is_suspended) {
		...
		ret = ili_report_handler(chip_data);
	...
	if (ret == P5_X_DEMO_PALM_PACKET_ID) {
		ILI_INFO("PALM start !\n");
		return IRQ_PALM;
	}
	return IRQ_TOUCH;
...
static void ilitek_rate_white_list_ctrl(void *chip_data, int value)
{
	...
	cmd[0] = 0x01;
	cmd[1] = 0x2F;
	cmd[2] = 0x00; /*0x00: Set Report Rate, 0x01: Get Report Rate*/
	switch (value) {
	case 120: /* 120Hz */
		cmd[3] = 0x78;
	...
	case 240: /* 240Hz */
		cmd[3] = 0xF0;
		ILI_INFO("report rate 240 hz\n");
		break;
	default:
		ILI_ERR(" report rate not support !!\n");
		goto out;
	}
	...
	if (retval < 0) {
		...
		ILI_INFO("Report rate change success !\n");
	}
out:
	mutex_unlock(&ilits->touch_mutex);
}
...
int ilitek7807s_spi_probe(struct spi_device *spi)
{
	...
	ts->com_test_data.chip_test_ops = &ilitek_7807_test_ops;
...
static int __init tp_driver_init_ili_7807s(void)
{
	int res = 0;
	ILI_INFO("%s is called\n", __func__);
...
late_initcall(tp_driver_init_ili_7807s);
#else
...
MODULE_DESCRIPTION("Touchscreen ili7807s Driver");
MODULE_LICENSE("GPL");
