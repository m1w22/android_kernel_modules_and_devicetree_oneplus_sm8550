...
#define MAIN_WLAN		WLAN0_INDEX
#define SECOND_WLAN		WLAN1_INDEX

#define MAIN_WLAN_MARK		WLAN0_MARK
#define SECOND_WLAN_MARK	WLAN1_MARK
...
#define TIMESTAMP_ERROR_THRESHOLD	10
...
static u32 get_skb_mark_by_weight(void)
{
	...
	u32 sla_random = get_random_u32() & 0x7FFFFFFF;
#else
	...
	enum ip_conntrack_info ctinfo)
{
	...
	if (oplus_app_type > 0 && oplus_app_type < GAME_NUM) {
		...
		if (iph &&
			...
			/*
			if (GAME_WZRY == game_index && oplus_sla_lobby_status != 1 &&
				IPPROTO_TCP == iph->protocol) {
				return SLA_SKB_ACCEPT;
			}
			*/
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(6, 1, 0))
	.resv_start_op = __OPLUS_SLA_CMD_MAX,
#endif
};
