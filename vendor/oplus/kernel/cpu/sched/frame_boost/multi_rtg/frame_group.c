...
static void add_task_to_frame_group(struct frame_group *grp, struct task_struct *task)
{
	...
	if (!is_single_bit) {
		ofb_err("line:[%d]: task=%s, pid=%d, tgid=%d, prio=%d, fbg_state=%d, fbg_cur_group_id:%d, group_id:%d, cpu=%d\n",
				__LINE__, task->comm, task->pid, task->tgid, task->prio, cur_fbg_state, ots->fbg_cur_group, grp->id, task_cpu(task));
	...
	unsigned int flags)
{
	...
	if ((frame_util >= vutil) ||
		(avai_buffer_count >= 3) ||
		((avai_buffer_count == 2) && (curr_putil < prev_putil || prev_putil < 100) && (curr_putil < (vutil >> 1))) ||
...
bool fbg_rt_task_fits_capacity(struct task_struct *tsk, int cpu)
{
	...
	grp_util = raw_util + schedtune_grp_margin(raw_util, grp->stune_boost[BOOST_SF_IN_GPU] ?
			grp->stune_boost[BOOST_SF_MIGR_GPU] : grp->stune_boost[BOOST_SF_FREQ_NONGPU]);
