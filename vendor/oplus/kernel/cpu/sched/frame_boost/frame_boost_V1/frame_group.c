...
static DEFINE_RAW_SPINLOCK(inputmethod_fbg_lock);
static struct frame_group inputmethod_frame_boost_group;
...
static inline struct frame_group *task_get_frame_group(struct oplus_task_struct *ots)
{
	if (ots->fbg_state & FRAME_COMPOSITION)
		...
		return &inputmethod_frame_boost_group;
	else
...
static inline int get_frame_group_id(struct frame_group *grp)
{
	...
	else if (grp == &inputmethod_frame_boost_group)
		return INPUTMETHOD_FRAME_GROUP_ID;
	else /* grp == &default_frame_boost_group */
		return DEFAULT_FRAME_GROUP_ID;
}

void id_get_group(int grp_id, struct frame_group **grp, raw_spinlock_t **lock)
{
...
static inline void frame_grp_with_lock_assert(struct frame_group *grp)
{
	if (grp == &sf_composition_group)
		...
		lockdep_assert_held(&inputmethod_fbg_lock);
	else /* grp == &default_frame_boost_group */
...
int get_frame_group_ui(int grp_id)
{
	if (grp_id == INPUTMETHOD_FRAME_GROUP_ID)
		return inputmethod_frame_boost_group.ui_pid;
	else
...
void set_frame_group_window_size(unsigned int window_size)
{
	...
	grp = &inputmethod_frame_boost_group;
	raw_spin_lock_irqsave(&inputmethod_fbg_lock, flags);
	grp->window_size = window_size;
	raw_spin_unlock_irqrestore(&inputmethod_fbg_lock, flags);
}
...
int rollover_frame_group_window(int group_id)
{
	...
	id_get_group(group_id, &grp, &lock);
...
static struct oplus_sched_cluster *best_cluster(struct frame_group *grp)
{
	...
	if (unlikely(sysctl_frame_boost_debug)) {
		pref_cpus_systrace_c(grp->ui_pid, cpumask_first(&best_cluster->cpus));
		if (grp->available_cluster)
			avai_cpus_systrace_c(grp->ui_pid, cpumask_first(&grp->available_cluster->cpus));
		else
			avai_cpus_systrace_c(grp->ui_pid, 0);
	}
...
unlock_fbg:
	...
	*util = max(raw_util, boosted_policy_util);
	...
	if (unlikely(sysctl_frame_boost_debug)) {
		...
		cpu_util_systrace_c(boosted_policy_util * 10000 + curr_util, first_cpu, "cfs_policy/curr_util");
	}
	...
	*util = max(*util, boosted_policy_util);
	...
	if (unlikely(sysctl_frame_boost_debug)) {
		int boost_sf;
		unsigned long curr_util = policy_util ? atomic64_read(&grp->curr_util) : 0;
		cpu_util_systrace_c(boosted_policy_util * 10000 + curr_util, first_cpu, "rt_policy/curr_util");
		if (stune_boost[BOOST_SF_IN_GPU])
			boost_sf = stune_boost[BOOST_SF_FREQ_GPU] * 1000 + stune_boost[BOOST_SF_MIGR_GPU];
		else
			boost_sf = stune_boost[BOOST_SF_FREQ_NONGPU] * 1000 + stune_boost[BOOST_SF_MIGR_NONGPU];
		val_systrace_c(boost_sf, "rt_boost_freq/migr");
	}
	...
	policy_util = 0;
	if (valid_freq_querys(query_cpus, grp))
		policy_util = atomic64_read(&grp->curr_util);

	raw_spin_unlock_irqrestore(&inputmethod_fbg_lock, flags);
	*util = max(*util, policy_util);

	if (unlikely(sysctl_frame_boost_debug)) {
		cpu_util_systrace_c(policy_util * 10000 + raw_util, first_cpu, "inputmethod/raw_util");
	}
...
void inputmethod_update_cpufreq(struct task_struct *tsk)
{
	struct frame_group *grp = &inputmethod_frame_boost_group;
	unsigned long flags;
	bool need_update = false;
	struct rq *rq = NULL;
	u64 wallclock = fbg_ktime_get_ns();
	...
	if (should_update_cpufreq(wallclock, grp, &inputmethod_fbg_lock)) {
		atomic64_set(&grp->last_freq_update_time, wallclock);
		need_update = true;
	}
...
EXPORT_SYMBOL_GPL(inputmethod_update_cpufreq);

void inputmethod_set_boost_start(void)
{
	struct frame_group *grp = &inputmethod_frame_boost_group;
	struct task_struct *ui;
	unsigned long flags;
...
unlock:
	...
	if (ui) {
		inputmethod_update_cpufreq(ui);
		put_task_struct(ui);
	}
}
EXPORT_SYMBOL_GPL(inputmethod_set_boost_start);
...
skip:
	if (need_freq_update) {
		...
		else if (inputmethod_part)
			inputmethod_update_cpufreq(tsk);
	}
}
	...
	struct task_struct *next, struct rq *rq)
{
	...
	if (unlikely(sysctl_frame_boost_debug) && likely(prev != next)) {
...
{
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_FAKE_CAP)
	if (eas_opt_enable && !force_apply_fake_enable)
...
int frame_group_init(void)
{
	...
	/* Inputmethod frame group initialization */
	grp = &inputmethod_frame_boost_group;
	INIT_LIST_HEAD(&grp->tasks);
	grp->window_size = NSEC_PER_SEC / DEFAULT_FRAME_RATE;
	grp->window_start = 0;
