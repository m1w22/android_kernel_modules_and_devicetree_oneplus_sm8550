...
static void count_ux_allocstall(void *data, int order, gfp_t gfp_flags)
{
...
static int register_zram_opt_vendor_hooks(void)
{
	...
	*/
	...
	ret = register_trace_mm_vmscan_direct_reclaim_begin(count_ux_allocstall, NULL);
	if (ret != 0) {
		pr_err("register_trace_android_vh_test_ux_task failed! ret=%d\n", ret);
		goto out;
	}
